...
y
```
- 虽然和普通继承的布局无差别，但是因为基类没有虚函数所以没有虚表，所以基类子对象的地址就不包含虚表了。

总结：
- 由这五个例子已经可以基本窥一斑而知全豹了（一定注意这是当前环境特定的是不可移植的）：
    - 虚指针放在对象或者子对象最开头，要具有虚函数才从虚指针开始，没有虚函数直接就从第一个成员开始了。
    - 单继承没有虚函数的基类：派生类如果引入了虚函数，那么虚指针在最前，派生类对象地址指向其虚指针，但基类子对象不包含虚指针，从其成员开始，然后派生类成员。如果派生类不引入虚函数，那么都不含虚指针，都从基类成员直接开始。
    - 单继承具有虚函数的基类：基类子对象与派生类对象拥有共同的地址，共用同一个虚指针，基类子对象成员在前，派生类成员在后。
